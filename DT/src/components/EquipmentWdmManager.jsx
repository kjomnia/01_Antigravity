...
import { Search, Save, RotateCcw, Database, Cpu, Printer } from 'lucide-react';
import { exportDataFile } from '../utils/excelUtils';

const EquipmentWdmManager = ({ inputRows, setInputRows, onReset }) => {
    console.log(`[EquipmentWdm] Rendering with ${inputRows?.length} rows`);
    ...
    const COLUMN_DEFS = [
        ...
        { key: 'eqQr', label: '장비QR' },
        { key: 'tid', label: 'TID' },
        { key: 'eqName', label: '장비명' },
    ];

    // 그리드 데이터 변경 핸들러
    const handleGridChange = (index, key, value) => {
        try {
            console.log(`[EquipmentWdm] Change - idx: ${index}, key: ${key}, val: ${value}`);
            ...
            console.error("[EquipmentWdm] handleGridChange error:", error);
        }
    };
    ...
    const handleExport = () => {
        ...
        exportDataFile(exportData, `장비WDM_${new Date().getTime()}`, 'xls');
    };
    ...
    const handleReset = () => {
        if (confirm("입력된 내용을 모두 초기화하시겠습니까?")) {
            console.log("[EquipmentWdm] Reset called");
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            ...
            if (firstInput) {
                console.log("[EquipmentWdm] Focus restored");
                firstInput.focus();
    ...
    return (
        <div className="flex h-full gap-4 p-4 bg-gray-100 items-stretch" style={{ minHeight: 'calc(100vh - 180px)' }}>
            {/* LEFT & CENTER: Input Area - Full Width */}
            <div className="flex-1 flex flex-col gap-4">
                ...
                <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-gray-800 text-lg">장비wdm 입력</span>
                    </div>
